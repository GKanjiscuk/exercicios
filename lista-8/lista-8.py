...
for palavra in txtB:
    if len(palavra) == 7 and palavra[-1] not in zumbi:
        verbos += 1
print(f'B)  O txtB possui {verbos} verbos')
verbos1P = 0
for palavra in txtB:
    if len(palavra) == 7 and palavra[0] not in zumbi and palavra[-1] not in zumbi:
        verbos1P += 1
print(f'C)  O txtB possui {verbos1P} verbos em primeira pessoa: ')
...
def troca(s):
    resp = ''
    for letra in s: 
        resp = resp + normal[ordem.index(letra)] 
        #.index() devolve a posição
    return resp
txtA.sort(key=troca)
print(txtA)
txtB.sort(key=troca)
...
print('')
print('Números mágicos txtA:')
for palavra in txtA:
    if valor(palavra) % 42 == 0 and len(palavra) == len(set(palavra)):
        print (palavra)
print('')
print('Números mágicos txtB:')
for palavra in txtB:
    if valor(palavra) % 42 == 0 and len(palavra) == len(set(palavra)):
        print (palavra)
